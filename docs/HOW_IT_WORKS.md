...
| 2 | Admin | `POST /users` | Creates a regular user, starts inactive |
...
**Consumer** — a registration that proves a user is authorised to act on behalf of a given `target_uri`. Optional — only needed if you want to query notifications filtered by target.
